...
class TestETLProcessor(unittest.TestCase):
    ...
    def test_setup_db_initial_count(self, mock_persistent_client, mock_logger):
        ...
        mock_logger.info.assert_any_call("START: Initial Chroma collection has 5 vectors. Each vector represents one page of a rss feed.")
    ...
    def test_read_feed_no_rss_urls(self, mock_logger):
        ...
        mock_logger.error.assert_called_with("No RSS URLs provided. Existing application. Sad!")
    ...
    def test_read_feed_bozo_exception(self, mock_feedparser, mock_logger):
        ...
        mock_logger.error.assert_called_with("Error parsing feed: ", mock_feed.bozo_exception)
    ...
    def test_read_feed_success(self, mock_feedparser, mock_logger):
        ...
        mock_logger.info.assert_any_call(f"Reading feed Title: {mock_feed.feed.title}, feed has {len(mock_feed.entries)} entries")
    ...
    def test_document_upsert_success(self, mock_persistent_client, mock_logger):
        ...
        self.etl_processor.documents = {doc_uid: MagicMock(doc_id=doc_uid, metadata={"link": "link"}, text="Test Text")}
